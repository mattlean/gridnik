...
describe('calcRightLeftMargins', () => {
  test('Successfully calculate right & left margin sum with valid margins', () => {
    ...
    const result = calcRightLeftMargins(canvasState, calcState)
...
describe('calcPillarWidth', () => {
  test('Successfully colCalcState calculate with parameters similar to Bootstrap 1920x1080 grid', () => {
    const canvasState = {
      canvasWidth: 1920,
    }
    let colCalcState = {
    ...
    const calcState = normalizeColData(colCalcState)
    const results = calcPillarWidth(canvasState, calcState, 'calcColWidth')
    colCalcState = denormalizeColData(calcState)
    ...
    expect(colCalcState).toMatchObject({
      cols: 12,
      colWidth: 81.25,
      colGutterWidth: 15,
      rightMargin: 390,
